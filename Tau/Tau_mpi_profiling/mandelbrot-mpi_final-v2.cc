...
int main(int argc, char* argv[]) {
    ...
    int aaSide = std::sqrt(static_cast<double>(aaSamples));
    ...
    MPI_Gather(&red[0], (end_row - start_row) * WIDTH, MPI_INT, &all_red[0], (end_row - start_row) * WIDTH, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&green[0], (end_row - start_row) * WIDTH, MPI_INT, &all_green[0], (end_row - start_row) * WIDTH, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Gather(&blue[0], (end_row - start_row) * WIDTH, MPI_INT, &all_blue[0], (end_row - start_row) * WIDTH, MPI_INT, 0, MPI_COMM_WORLD);
